...
To launch the entire microservices stack (Postgres, Kafka, Zookeeper, Schema Registry, Node.js Backend, Python ML Inference) with a single command:
...
If you prefer running services locally or through your IDE (e.g. VS Code):
...
#### Step 3: Run Node.js Express Backend
...
Send raw transaction batches to the Node.js ingestion endpoint:
...
Retrieve all flagged risk records from the Node.js Backend:
...
| :--- | :--- | :--- | :--- |
| **Server Port** | `Backend/.env` / Environment | `8080` (Docker) / `8085` (Local) | HTTP Port for Express REST Server |
| **PostgreSQL Host/Port** | `Backend/.env` / Environment | `localhost:5433` | Database Connection details |
| **Kafka Bootstrap** | `Backend/.env` / Environment | `localhost:9092` (Host) / `kafka:29092` (Docker) | Event Streaming Broker Host |
...
- **Solution**: Either stop the conflicting process or set `PORT=8085` in `Backend/.env` or environment variables.
